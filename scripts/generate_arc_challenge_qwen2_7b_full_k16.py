...
def parse_choice_content(content):
    ...
    if all(
        ...
        len(normalize_choice_text(label)) == 1
        and normalize_choice_text(label).isalpha()
        for label in labels
    ):
        positional_aliases = [
            str(index + 1)
    ...
    if (
        ...
        and alias_matches[0] != text_matches[0]
    ):
        return None

    if len(alias_matches) == 1:
        index = alias_matches[0]
        return {
            "index": index,
            "label": str(labels[index]),
            "method": (
                "boxed_choice_positional_alias"
            ),
        }
...
def main():
    ...
    if "--preflight-only" in __import__("sys").argv[1:]:
        print("Preflight-only 模式完成；不会加载模型或生成候选。")
        return
