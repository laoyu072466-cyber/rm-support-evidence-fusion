...
OUTPUT_PATH = (
    ROOT / "data/manifests/"
    "reliability_aware_math_fixed_risk_gate_train_pilot_v3.json"
)
PROTOCOL_TAG = (
    "reliability-aware-math-fixed-risk-gate-protocol-v3"
)
...
def preflight(require_frozen):
    print(
        "===== Reliability-Aware Math Fixed-Base Risk Gate 预检 ====="
    )
...
def main():
    ...
    print()
    print(
        "===== 训练 Reliability-Aware Math Fixed-Base Risk Gate ====="
    )
    ...
    selected[
        "accept_all_reproduces_fixed_gate"
    ] = True
    ...
    result = {
        "version": (
            "reliability_aware_math_fixed_risk_"
            "gate_train_pilot_v3"
        ),
        ...
        "protocol": {
            ...
            "base_proposal": (
                ...
                "fixed-gate RM-Support"
            ),
            "residual_gate_scope": (
                "only switches already accepted "
                "by the fixed gate"
            ),
            "training_target": (
            ...
            "class_weight": None,
            "decision_policy": (
                "accept fixed-gate proposal unless "
                "estimated harm probability exceeds "
                "threshold"
            ),
            "accept_all_threshold": 1.0,
            "accept_all_reproduces": (
                "fixed_gate"
            ),
            "pilot_damage_limit": (
                cluster.PILOT_DAMAGE_LIMIT
        ...
        "decision": (
            "freeze_math_fixed_base_harm_risk_"
    ...
    print()
    print(
        "RELIABILITY_MATH_FIXED_RISK_GATE_V3_FREEZE_READY"
    )
