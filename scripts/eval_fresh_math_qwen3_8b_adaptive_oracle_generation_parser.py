...
OUTPUT_PATH = (
    ROOT
    / "data/manifests/"
    "fresh_math_2026_qwen3_8b_adaptive_oracle_generation_parser.json"
)
...
def main():
    ...
    for row in candidates:
        ...
        if (
            frozen_prediction is None
            or not str(frozen_prediction).strip()
        ):
            prediction = None
            method = "unparsed"
        else:
            ...
            if frozen_prediction.startswith(
                ("number:", "expr:")
            ):
    ...
    output = {
        "version": (
            "fresh_math_2026_qwen3_8b_adaptive_oracle_generation_parser_v1"
        ),
        "label_reveal": {
        ...
        "parser": {
            ...
            "family": "math",
            "rule": (
                "use the answer extracted during "
                "generation before label reveal; "
                "no post-hoc text fallback"
            ),
        },
        "datasets": datasets,
        "pooled": pooled,
