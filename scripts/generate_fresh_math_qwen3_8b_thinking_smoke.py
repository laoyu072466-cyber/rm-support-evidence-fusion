...
CONFIG_PATH = (
    ROOT
    / "configs/fresh_math_2026_qwen3_8b_thinking_smoke.json"
)
PARTS_DIR = (
    ROOT
    / "outputs/fresh_math_2026/qwen3_8b_thinking_smoke_parts"
)
OUTPUT_PATH = (
    ROOT
    / "outputs/fresh_math_2026/"
    "qwen3_8b_thinking_smoke_candidates.jsonl"
)
MANIFEST_PATH = (
    ROOT
    / "data/manifests/"
    "fresh_math_2026_qwen3_8b_thinking_smoke.json"
)
...
def first_final_boxed(text):
    """
    只接受 </think> 之后出现的首个完整 boxed。
    防止在思考阶段提前停止或解析中间结果。
    """
    marker = "</think>"
    think_end = text.rfind(marker)

    if think_end < 0:
        return None, None

    final_start = think_end + len(marker)
    content, local_end = first_valid_boxed(
        text[final_start:]
    )
    ...
    return content, final_start + local_end


def parse_final_answer(text):
    """
    优先只解析 </think> 后的最终回答；
    没有正常闭合 thinking 时保留原解析器作为回退。
    """
    marker = "</think>"
    think_end = text.rfind(marker)
    ...
    if think_end >= 0:
        final_text = text[
            think_end + len(marker):
        ]
        answer, method = parse_answer(final_text)
...
class ThinkingFinalAnswerStop(StoppingCriteria):
    """
    对每个候选独立停止：
    必须先看到 </think>，再看到完整非空 boxed。
    每步只解码尾部 token，避免反复解码整条长序列。
    """
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for index in range(batch_size):
            ...
            if (
                not self.think_ended[index]
                and "</think>" in tail_text
            ):
                self.think_ended[index] = True

            if self.think_ended[index]:
...
def main():
    ...
    if missing:
        ...
        print("加载 Qwen3-8B Thinking……", flush=True)
    ...
    manifest = {
        "version": (
            "fresh_math_2026_qwen2_7b_k16_candidates_v1"
        ),
        "generator_track": "B",
        "labels_loaded": False,
