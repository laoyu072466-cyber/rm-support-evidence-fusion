...
OUTPUT_PATH = (
    ROOT / "data/manifests/"
    "reliability_aware_math_gate_train_pilot_v1.json"
)
PROTOCOL_TAG = (
    "reliability-aware-math-gate-protocol-v1"
)
...
def preflight(require_frozen):
    print(
        "===== Reliability-Aware Math Gate 预检 ====="
    )
...
def main():
    ...
    print()
    print(
        "===== 训练 Reliability-Aware Math Gate ====="
    )
    ...
    print()
    print("===== Reliability 训练样本 =====")
    print("decisive switches:", len(train_y))
    print("beneficial:", int(np.sum(
        train_y == 1
    )))
    ...
    result = {
        ...
        "training": {
            "decisive_switches": len(
                train_y
            ),
            "beneficial": int(np.sum(
                train_y == 1
            )),
            "harmful": int(np.sum(
                train_y == 0
        ...
        "pilot_macro": {
            ...
            "reliability_vs_raw_top1": (
                reliability_macro["top1"]
                - raw_macro["top1"]
            ),
            "reliability_vs_fixed_top1": (
                reliability_macro["top1"]
                - fixed_macro["top1"]
            ),
            "reliability_vs_ungated_top1": (
                reliability_macro["top1"]
                - ungated_macro["top1"]
            ),
        },
        ...
        "decision": (
            "freeze_math_trained_reliability_"
            "gate_before_any_new_benchmark"
        ),
    }
    ...
    print()
    print(
        "RELIABILITY_MATH_GATE_FREEZE_READY"
    )
